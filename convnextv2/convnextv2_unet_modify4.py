...
class ConvNeXtV2_unet(nn.Module):
    ...
    def __init__(
        ...
        self.sff_stage.append(DWTconvfuse(dims[0]))
        self.sff_stage.append(DWTconvfuse(dims[1]))
        self.sff_stage.append(DWTconvfuse(dims[2]))
        self.sff_stage.append(DWTconvfuse(dims[3]))
        ...
        self.decoder = Decoder(dims, self.decoder_dim, dropout=0.1, window_size=8, num_classes=num_classes)
    ...
    def encoder(self, x: Tensor, y:Tensor) -> Tuple[Tensor, List[Tensor]]:
        ...
        for i in range(self.num_stage-1):
            ...
            features.append(self.sff_stage[i](x, y))
        x = self.downsample_layers[-1](x)
        y = self.downsample_layers2[-1](y)
        features.append(self.sff_stage[-1](x, y))
